...
// Asks the device for its REAL current location. Nothing else is ever added:
// success -> the current location is set/replaced (never duplicated);
// failure -> an error is shown and nothing changes.
...
// Makes sure every list a view relies on is really an array, so one badly
// filled product can never break a page.
function normalizeProduct(p) {
  const arr = v => Array.isArray(v) ? v : [];
  p.id = String(p.id);
  p.images = arr(p.images);
  p.colors = arr(p.colors);
  p.colorNames = arr(p.colorNames);
  p.sizes = arr(p.sizes);
  p.features = arr(p.features);
  p.specs = arr(p.specs);
  p.variations = arr(p.variations);
  p.tags = arr(p.tags);
  return p;
}

/* Wishlist + bag are saved in localStorage (per signed-in user) so they
   survive refreshes, sign-outs and closing the browser. Ids are always
   stored as strings so numeric and UUID product ids both work. */
const shopKey = kind => `clarehil_${kind}_${state.user.id || "guest"}`;

function saveShopState() {
  try {
    localStorage.setItem(shopKey("wishlist"), JSON.stringify(Array.from(state.wishlist)));
    localStorage.setItem(shopKey("cart"), JSON.stringify(state.cart));
  } catch (e) { console.warn("Could not save wishlist/bag locally", e); }
}

function loadShopState() {
  try {
    const w = JSON.parse(localStorage.getItem(shopKey("wishlist")) || "[]");
    state.wishlist = new Set(Array.isArray(w) ? w.map(String) : []);
  } catch (e) { state.wishlist = new Set(); }
  try {
    const c = JSON.parse(localStorage.getItem(shopKey("cart")) || "[]");
    state.cart = Array.isArray(c)
      ? c.filter(x => x && x.id != null).map(x => ({ id: String(x.id), quantity: Math.max(1, Math.floor(Number(x.quantity)) || 1) }))
      : [];
  } catch (e) { state.cart = []; }
}

const wishHas = id => state.wishlist.has(String(id));
...
async function loadProductsFromSupabase() {
  ...
  products = (data || []).map(window.mapProduct).map(normalizeProduct);
  // Drop saved items whose product was deleted/unpublished by the admin.
  const known = new Set(products.map(p => String(p.id)));
  state.cart = state.cart.filter(l => known.has(String(l.id)));
  state.wishlist = new Set(Array.from(state.wishlist).filter(id => known.has(String(id))));
  saveShopState();
  render();
}
...
const cartLines = () => state.cart.map(line => ({ line, item: product(line.id) })).filter(x => x.item);
...
// Re-shows the current message after the page has been redrawn.
function showToast() {
  const t = $("#toast");
  if (!t || Date.now() >= toastUntil) return;
...
function go(hash) {
  ...
  if (location.hash === target) render();   // same page: no hashchange fires, so redraw (also closes the menu)
...
function paint(inner, opts = {}) {
  $("#app").innerHTML = `
    ...
    ${lightbox()}
  `;
  try { bindChrome(); } catch (err) { console.error(err); }
  showToast();
}
...
function productCard(p) {
  return `<article class="product-card">
    <button class="heart-btn ${wishHas(p.id) ? "active" : ""}" data-wish-id="${p.id}" aria-label="Toggle wishlist">${icon("heart")}</button>
  ...
  return `
    ...
    <div class="detail-info">
      ...
      <div class="purchase-row">
        ...
        <button class="wish-toggle ${wishHas(p.id) ? "active" : ""}" id="wishToggle" aria-label="Toggle wishlist">${icon("heart")}</button>
      </div>
    ...
    <div>${lines.map(({ line, item }) => `
      <article class="cart-item">
        ...
        <div class="cart-item-info">
          ...
          <h3>${item.name}</h3>
          <p>${[item.colorNames[0], item.sizes[0]].filter(Boolean).join(" · ")}</p>
          <div class="cart-qty">
...
function viewWishlist() {
  const items = products.filter(p => wishHas(p.id));
  return `
...
function viewAddresses() {
  return `
    ...
    <div class="block" style="padding-top:14px;"><button class="btn-secondary btn-block" id="addAddress" ${locationBusy ? "disabled" : ""}>${locationBusy ? "Getting your location…" : "Use my current location"}</button></div>
  `;
}
...
function bindDetail(p) {
  ...
  on("[data-qty]", "click", e => { state.quantity = Math.max(1, Math.min(99, state.quantity + Number(e.currentTarget.dataset.qty))); render(); });
...
function bindAddresses() {
  ...
  // Always fetches the user's real location. It never adds placeholder addresses.
  $("#addAddress")?.addEventListener("click", () => {
    requestShippingLocation(() => toast("Location updated"));
  });
}
...
function addToBag(id, qty = 1) {
  ...
  if (!item) { toast("This product is no longer available"); return; }
  const line = state.cart.find(i => String(i.id) === id);
  if (line) line.quantity = Math.min(99, line.quantity + qty);
  else state.cart.push({ id, quantity: Math.min(99, qty) });
  saveShopState();
...
function changeCartQty(id, delta) {
  ...
  const line = state.cart.find(i => String(i.id) === id);
  if (!line) return;
  line.quantity = Math.max(1, Math.min(99, line.quantity + delta));   // "Remove" is what deletes an item
  saveShopState();
  render();
}
...
// One click handler for the whole document. Because it doesn't depend on
// being re-attached after every redraw, these buttons keep working no matter
// how many pages the user visits or how long they stay.
document.addEventListener("click", e => {
  const t = e.target instanceof Element ? e.target : null;
  if (!t) return;

  const wish = t.closest("[data-wish-id]");
  if (wish) { e.preventDefault(); toggleWish(wish.dataset.wishId); return; }

  const add = t.closest("[data-add]");
  if (add) { e.preventDefault(); addToBag(add.dataset.add); return; }

  const qtyBtn = t.closest("[data-cq]");
  if (qtyBtn) { e.preventDefault(); changeCartQty(qtyBtn.dataset.cid, Number(qtyBtn.dataset.cq)); return; }

  const rem = t.closest("[data-remove]");
  if (rem) { e.preventDefault(); removeFromCart(rem.dataset.remove); return; }

  // Product page buttons: the product is identified from the page address.
  const onProduct = location.hash.replace(/^#/, "").startsWith("product/");
  const pid = onProduct ? decodeURIComponent(location.hash.replace(/^#/, "").split("/")[1] || "") : "";
  if (pid && t.closest("#addBag")) { e.preventDefault(); addToBag(pid, state.quantity); return; }
  if (pid && t.closest("#wishToggle")) { e.preventDefault(); toggleWish(pid); return; }
});
...
// Only a real page change jumps to the top. Re-renders caused by typing,
// tapping a button, toggling a setting etc. keep the scroll position and
// keep the cursor in the field the user was typing in.
let lastRenderedHash = null;
function render() {
  const hash = location.hash.replace(/^#/, "");
  const navigated = hash !== lastRenderedHash;
  lastRenderedHash = hash;

  const oldScreen = $(".screen");
  const keepWindowY = window.scrollY;
  const keepScreenY = oldScreen ? oldScreen.scrollTop : 0;

  const active = document.activeElement;
  const activeId = active && active.id && /^(INPUT|TEXTAREA)$/.test(active.tagName) ? active.id : "";
  let selStart = null, selEnd = null;
  if (activeId) { try { selStart = active.selectionStart; selEnd = active.selectionEnd; } catch (e) {} }

  try {
    renderView(hash);
  } catch (err) {
    console.error("Page failed to draw, showing a safe screen instead.", err);
    try {
      paint(`<div class="cart-empty"><h2>Something went wrong.</h2><p>This page couldn't be displayed.</p><button class="btn-primary" data-nav="home">Back to home</button></div>`, { tab: "home" });
  ...
  if (navigated) {
    ...
    window.scrollTo({ top: keepWindowY, left: 0, behavior: "instant" });
    $(".screen")?.scrollTo({ top: keepScreenY, left: 0, behavior: "instant" });
  }
